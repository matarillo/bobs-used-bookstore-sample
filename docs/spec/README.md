...
**この写像の正は [story-map.md §5](../product/story-map.md) の「派生 UC-」列である。**
「由来」欄はそこへの参照であり、食い違ったら story-map を正とする。
まだ `UC-` に落ちていない `STORY-` は、その列が空であることでしか表せない——
だから写像は要求層に置く（同じ事実を二か所に置かない／[文書体系 §6.1](../README.md)）。
