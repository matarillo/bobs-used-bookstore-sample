...
| 注文対象の抽出 | 在庫のある購入明細が 1 件以上ある（[INV-CART-08](aggregate-shopping-cart.md)） |
| 在庫の引き当て | 在庫を超える引き当ては拒否される（[INV-BOOK-03](aggregate-book.md)） |
...
| プロフィールの登録・更新 | **顧客を作る** |
| 注文の確定 | **失敗する** |
| 買取オファーの申込 | （検査されていない — [ISSUE-04](../issues.md)） |
...
| 要素 | 内容 |
| --- | --- |
| データベース接続情報 (DbSecrets) | 接続先・資格情報の保持。基盤設定である（[ISSUE-16](../issues.md)） |
