...
| **売上絞り込みが売上計上対象と全状態で一致する** | [読み取りモデル](read-models.md) §3.3 |
| 納品予定日が UTC から 7 日後 | `RULE-PERIOD-03` |
| 納期超過が未配達かつ期日超過で真、配達済・取消済で偽 | `RULE-PERIOD-04` |
| 期日前は納期超過にならない | `RULE-PERIOD-04` |
| **納期超過絞り込みが納期超過判定と全状態・両方の期日で一致する** | [読み取りモデル](read-models.md) §3.3 |
...
| 統計が取得できないとき空の統計が返る | [読み取りモデル](read-models.md) |
...
| `INV-CART-06b` | **[不成立]**。移動操作に**購入明細の識別子を渡す経路が検証されていない**。この経路が黙って明細を削除することは、既存のテストでは発見できない（[ISSUE-21](../issues.md)） |
