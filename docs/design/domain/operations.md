...
画面・遷移・入力書式は扱わない。**何が起き、何が保証されるか**のみを記す。

利用者が画面を通じて行うことは [ユースケース](../../spec/use-cases/) が定める。
一つのユースケースが複数の操作を呼ぶことも、ユースケースを持たない操作もある。

## 1. 顧客が起こす操作

| 識別子 | 操作 | 主に触れる集約 |
| --- | --- | --- |
| `OP-CUST-01` | プロフィールを登録・更新する | AGG-CUSTOMER |
| `OP-CUST-02` | 配送先住所を登録する | AGG-ADDRESS, AGG-CUSTOMER |
| `OP-CUST-03` | 配送先住所を更新する | AGG-ADDRESS |
| `OP-CUST-04` | 配送先住所を削除する | AGG-ADDRESS |
| `OP-CUST-05` | 書籍を検索・閲覧する | AGG-BOOK |
| `OP-CUST-06` | 書籍を買い物かごに入れる | AGG-CART |
| `OP-CUST-07` | 書籍を欲しい物リストに入れる | AGG-CART |
| `OP-CUST-08` | 欲しい物リストの1件をかごへ移す | AGG-CART |
| `OP-CUST-09` | 欲しい物リストをすべてかごへ移す | AGG-CART |
| `OP-CUST-10` | かごの明細を削除する | AGG-CART |
| `OP-CUST-11` | 注文を確定する | AGG-ORDER, AGG-CART, AGG-BOOK, AGG-CUSTOMER |
| `OP-CUST-12` | 自分の注文を照会する | AGG-ORDER |
| `OP-CUST-13` | 自分の注文を取り消す | AGG-ORDER, AGG-BOOK |
| `OP-CUST-14` | 買取を申し込む | AGG-OFFER, AGG-CUSTOMER, AGG-REFDATA |
| `OP-CUST-15` | 自分の買取オファーを照会する | AGG-OFFER |

---

### OP-CUST-02 — 配送先住所を登録する
...
### OP-CUST-08 / OP-CUST-09 — 欲しい物リストからかごへ移す

| 項目 | 個別移動 (OP-CUST-08) | 一括移動 (OP-CUST-09) |
| --- | --- | --- |
...
| 識別子 | 操作 | 主に触れる集約 |
| --- | --- | --- |
| `OP-STAFF-01` | 参照データ項目を登録する | AGG-REFDATA |
| `OP-STAFF-02` | 参照データ項目を改名する | AGG-REFDATA |
| `OP-STAFF-03` | 参照データを一覧・照会する | AGG-REFDATA |
| `OP-STAFF-04` | 書籍を手入力で登録する | AGG-BOOK, AGG-REFDATA |
| `OP-STAFF-05` | 書籍を更新する | AGG-BOOK, AGG-REFDATA |
| `OP-STAFF-06` | 買取オファーを承認する | AGG-OFFER |
| `OP-STAFF-07` | 買取オファーを却下する | AGG-OFFER |
| `OP-STAFF-08` | 買取オファーの受領を確認する | AGG-OFFER |
| `OP-STAFF-09` | 買取オファーの支払を記録する | AGG-OFFER |
| `OP-STAFF-10` | 支払済オファーを棚入れする | AGG-BOOK, AGG-OFFER |
| `OP-STAFF-11` | 買取オファーを一覧・照会する | AGG-OFFER |
| `OP-STAFF-12` | 注文を受け付ける | AGG-ORDER |
| `OP-STAFF-13` | 注文を出荷する | AGG-ORDER |
| `OP-STAFF-14` | 注文の配達完了を記録する | AGG-ORDER |
| `OP-STAFF-15` | 注文を一覧・照会する | AGG-ORDER |
| `OP-STAFF-16` | ダッシュボードの統計を閲覧する | 読み取りモデル |

---

### OP-STAFF-02 — 参照データ項目を改名する
...
### OP-STAFF-04 / OP-STAFF-05 — 書籍を登録・更新する

| 項目 | 登録 (OP-STAFF-04) | 更新 (OP-STAFF-05) |
| --- | --- | --- |
...
graph LR
    subgraph C["顧客"]
        C1["OP-CUST-01〜04<br/>プロフィール・住所"]
        C2["OP-CUST-05〜10<br/>閲覧・かご"]
        C3["OP-CUST-11〜13<br/>注文"]
        C4["OP-CUST-14〜15<br/>買取申込"]
    end
    subgraph S["スタッフ"]
        S1["OP-STAFF-01〜03<br/>参照データ"]
        S2["OP-STAFF-04〜05<br/>書籍登録"]
        S3["OP-STAFF-06〜11<br/>オファー審査・棚入れ"]
        S4["OP-STAFF-12〜16<br/>注文履行・統計"]
    end
