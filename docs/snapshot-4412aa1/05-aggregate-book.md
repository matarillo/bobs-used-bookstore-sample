...
| ISBN | テキスト | ● | |
| 出版年 | 年 | ○ | |
| 分類 | [VO-CLASSIFICATION](03-building-blocks.md) | ● | 出版社・書籍種別・ジャンル・状態の4軸 |
| 概要 | テキスト | ○ | |
| 表紙画像 | 外部資源参照 | ○ | |
| 売価 | [VO-MONEY](03-building-blocks.md) | ● | |
| 在庫数 | [VO-QUANTITY](03-building-blocks.md) | ● | 0 を許す |
| 供給元オファー | 参照 | ○ | どの買取オファーから生まれたか。手入力の書籍では未設定 |
| 仕入原価 | [VO-MONEY](03-building-blocks.md) | ○ | 棚入れ時にオファーから写し取った金額。供給元オファーがなければ未設定 |
| （内部数値属性） | | ▲ | 売価・在庫数・仕入原価の素の数（[03](03-building-blocks.md) §8） |
...
（[13-read-models-and-statistics.md](13-read-models-and-statistics.md) §2.1）。
...
**支払完了かつ未棚入れのオファーからしか生まれない**（[INV-OFFER-05](09-aggregate-offer.md)）。
...
| 識別子 | 不変条件 | 強制レベル | 根拠 |
| --- | --- | --- | --- |
| `INV-BOOK-01` | 在庫数は 0 以上 | **[強制]** | [VO-QUANTITY](03-building-blocks.md) が生成時に保証 |
| `INV-BOOK-02` | 売価は 0 以上、通貨最小単位に丸められている | **[強制]** | [VO-MONEY](03-building-blocks.md) が生成時に保証 |
| `INV-BOOK-03` | 出庫は在庫数を超えられない | **[強制]** | 在庫を減らす操作が拒否する |
| `INV-BOOK-04` | 分類の4軸は検証済みである | **[強制]** | 分類は [VO-CLASSIFICATION](03-building-blocks.md) 経由でしか設定できない |
...
| `INV-BOOK-07` | 在庫の増減は在庫操作を通じて行われる | **[表明]** | 在庫数は外部から直接代入できる（[ISSUE-01](15-design-issues.md)） |
...
| 統計 | — | [RM-BOOK-STATS](13-read-models-and-statistics.md) |
