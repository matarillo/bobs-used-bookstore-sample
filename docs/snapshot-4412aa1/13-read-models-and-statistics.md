...
**別の条件として**提供している（[05-aggregate-book.md](05-aggregate-book.md) §8.1）。
...
**両者が同じ答えを返すことはテストで固定されている**（[14](14-specification-by-test.md)）。
...
| 事項 | ドメインの規定 |
| --- | --- |
| 「今月」の起点 | **規定あり**（当月 1 日 0 時、[RULE-PERIOD-02](12-invariants-and-rules.md)） |
| 売上計上の対象 | **規定あり**（注文集約の売上絞り込み） |
| 納期超過の判定 | **規定あり**（注文集約の納期超過絞り込み） |
| 「今月の注文数」の基準日 | **未定義**。注文日という属性がなく、基底の作成日時に依拠する（[ISSUE-11](15-design-issues.md)） |
