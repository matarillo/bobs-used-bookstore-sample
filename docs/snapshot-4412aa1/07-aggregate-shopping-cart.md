...
| 所属かご | 参照 | ● | |
| 書籍 | 参照 | ● | |
| 数量 | [VO-QUANTITY](03-building-blocks.md) | ● | 1 以上 |
| **購入意思 (WantToBuy)** | 真偽 | ● | 真＝買い物かご、偽＝欲しい物リスト |
| （内部数値属性） | | ▲ | 数量の素の数（[03](03-building-blocks.md) §8） |
...
両者が同一人物のものであるかは検証されない（[ISSUE-02](15-design-issues.md)）。
...
> **黙って削除される**（[ISSUE-21](15-design-issues.md)）。
...
| `INV-CART-06b` | 移動操作の対象は欲しい物明細でなければならない | **[不成立]** | 種別が検査されず、購入明細を渡すと削除される（[ISSUE-21](15-design-issues.md)） |
| `INV-CART-07` | 明細の集合はかごの振る舞いを通じてのみ変更される | **[表明]** | 明細の集合・数量・購入意思は外部から直接変更できる（[ISSUE-01](15-design-issues.md), [ISSUE-18](15-design-issues.md)） |
...
書籍が併せて読み込まれていなければ成立しない（[ISSUE-20](15-design-issues.md)）。
