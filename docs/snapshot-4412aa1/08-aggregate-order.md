...
| 所属注文 | 参照 | ● | 不可 | |
| 書籍 | 参照 | ● | 不可 | |
| 数量 | [VO-QUANTITY](03-building-blocks.md) | ● | **不可** | 1 以上 |
| **価格** | [VO-MONEY](03-building-blocks.md) | ● | **不可** | 注文時点の書籍の売価を写し取ったもの |
| **原価** | [VO-MONEY](03-building-blocks.md) | ○ | **不可** | 注文時点の書籍の仕入原価を写し取ったもの。書籍がオファー由来でなければ未設定 |
| （内部数値属性） | | ▲ | | 数量・価格・原価の素の数 |
...
> 注文サービスの責務である（[ISSUE-05](15-design-issues.md)）。
...
両者が同じ答えを返すことはテストで固定されている（[SPEC-ORDER](14-specification-by-test.md)）。
...
DDD の一般原則からの意図的な逸脱である。詳細は [ISSUE-03](15-design-issues.md)。
...
> 基底属性の作成日時に依拠している（[ISSUE-11](15-design-issues.md)）。
...
| 配送業者・追跡番号 | 概念なし |
| 返品・返金 | 概念なし。取消のみ |
| 税率の変動 | 定数。時期・地域による差異を表現できない（[ISSUE-10](15-design-issues.md)） |
| 送料・割引 | 概念なし |
